...
const startCronJobs = () => {
    ...
    node_cron_1.default.schedule('59 23 * * *', async () => {
        try {
            const admins = sqlite_1.db.prepare("SELECT id, name, phone FROM users WHERE role = 'admin' AND phone IS NOT NULL AND trim(phone) != ''").all();
            if (admins.length === 0) {
                console.log('[Cron] Daily report skipped: no admin user with phone number');
                return;
            }
            const today = new Date().toISOString().split('T')[0];
            const orders = sqlite_1.db.prepare("SELECT ticket_id, total, paid, created_at FROM orders WHERE date(created_at) = ?").all(today);
            const revenue = orders.reduce((s, o) => s + o.paid, 0);
            const expected = orders.reduce((s, o) => s + o.total, 0);
            const withdrawals = sqlite_1.db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM transactions WHERE date = ? AND type = 'withdrawal'").get(today).total;
            const msg = [
            ...
            for (const admin of admins) {
                await (0, whatsapp_service_1.sendWhatsAppMessage)(admin.phone, msg);
             ...
             VALUES (lower(hex(randomblob(16))), ?, 'salary_reminder', 'Salaires', 'Pensez à vérifier et payer les salaires du mois.')`).run(admin.id);
                    sqlite_1.db.prepare(`INSERT INTO notifications (id, user_id, type, title, body)
             VALUES (lower(hex(randomblob(16))), ?, 'withdrawal_reminder', 'Retraits', 'Retraits du jour: ${withdrawals.toFixed(2)} DH.')`).run(admin.id);
                }
                catch (e) {
    ...
    node_cron_1.default.schedule('0 * * * *', async () => {
        ...
        try {
            ...
            const readyForPickUpItems = sqlite_1.db.prepare(`
      ...
      `).all();
            ...
            for (const item of readyForPickUpItems) {
                ...
                const success = await (0, whatsapp_service_1.sendWhatsAppMessage)(clientPhone, message);
                if (success) {
                    sqlite_1.db.prepare("UPDATE order_items SET client_notified_at = datetime('now') WHERE id = ?").run(item.id);
                    // In-app notification for all admins: client reminder sent
                    try {
                        sqlite_1.db.prepare(`INSERT INTO notifications (id, user_id, type, title, body)
               VALUES (lower(hex(randomblob(16))), NULL, 'client_reminder', 'Rappel client', 'Ticket #${item.ticket_id} prêt pour ${item.client_name || 'client'}')`).run();
                    }
                    catch (e) {
            ...
            const stuckItems = sqlite_1.db.prepare(`
      ...
      `).all();
            ...
            for (const item of stuckItems) {
                if (!item.caissier_phone || !String(item.caissier_phone).trim())
                    continue;
                const articleRow = sqlite_1.db.prepare('SELECT name FROM articles WHERE id = ?').get(item.article_id);
                ...
                const success = await (0, whatsapp_service_1.sendWhatsAppMessage)(item.caissier_phone, message);
                if (success) {
                    sqlite_1.db.prepare("UPDATE order_items SET caissier_stuck_notified_at = datetime('now') WHERE id = ?").run(item.id);
                    // In-app notification for caissier: article stuck reminder
                    try {
                        if (item.created_by) {
                            sqlite_1.db.prepare(`INSERT INTO notifications (id, user_id, type, title, body)
                 VALUES (lower(hex(randomblob(16))), ?, 'article_reminder', 'Article en attente', '\"${articleName}\" (Ticket #${item.ticket_id}) en statut \"${statusLabel}\" depuis > 12h.')`).run(item.created_by);
                        }
                    }
                    catch (e) {
