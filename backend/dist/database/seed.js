...
function seedDB() {
    const catCount = sqlite_1.db.prepare("SELECT COUNT(*) as c FROM categories").get().c;
    if (catCount > 0)
        return;
    console.log("[seed] Seeding reference data...");
    sqlite_1.db.prepare(`INSERT INTO categories (id, name, name_ar, image, color) VALUES 
    ...
    ('maison', 'Maison', 'منزل', 'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?q=80&w=1470&auto=format&fit=crop', 'bg-green-500')`).run();
    sqlite_1.db.prepare(`INSERT INTO services (id, name, name_ar, multiplier, image, color) VALUES 
    ...
    ('lavage_repassage', 'Mix', 'غسيل وكي', 1.5, 'https://images.unsplash.com/photo-1517677208171-0bc6725a3e60?q=80&w=500&auto=format&fit=crop', 'text-blue-500')`).run();
    sqlite_1.db.prepare(`INSERT INTO users (id, name, role, pin_code, avatar, salary) VALUES 
    ('u1', 'Admin', 'admin', '1234', '🔐', 0),
    ('u2', 'Caissier', 'cashier', '2222', '🧑‍💼', 3000)`).run();
    sqlite_1.db.prepare(`INSERT INTO suppliers (id, name, name_ar, logo, contact) VALUES 
    ('sup1', 'Tapis Master', 'سجاد ماستر', 'https://picsum.photos/seed/sup1/100/100', ''),
    ('sup2', 'Clean Expert', 'كلين إكسبرت', 'https://picsum.photos/seed/sup2/100/100', '')`).run();
    sqlite_1.db.prepare(`INSERT INTO machines (id, name, name_ar, type, status, capacity) VALUES 
    ('m1', 'Lave-Linge 1', 'غسالة 1', 'washer', 'disponible', '7kg'),
    ('m2', 'Lave-Linge 2', 'غسالة 2', 'washer', 'disponible', '12kg'),
    ('m3', 'Sèche-Linge 1', 'مجفف 1', 'dryer', 'disponible', '15kg')`).run();
    sqlite_1.db.prepare(`INSERT INTO articles (id, category_id, name, name_ar, image, base_price) VALUES 
    ...
    ('couette', 'maison', 'Couette', 'لحاف', 'https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?q=80&w=500&auto=format&fit=crop', 80)`).run();
